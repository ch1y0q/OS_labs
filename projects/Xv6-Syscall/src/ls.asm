...
00000060 <fmtname>:
{
  ...
  return buf;
  c5:	bb d4 0d 00 00       	mov    $0xdd4,%ebx
  memset(buf+strlen(p), ' ', DIRSIZ-strlen(p));
  ...
  db:	05 d4 0d 00 00       	add    $0xdd4,%eax
  e0:	52                   	push   %edx
  e1:	6a 20                	push   $0x20
  e3:	50                   	push   %eax
 ...
 65c:	89 d1                	mov    %edx,%ecx
{
 ...
 689:	0f b6 92 ac 0a 00 00 	movzbl 0xaac(%edx),%edx
...
printf(int fd, const char *fmt, ...)
{
 ...
 715:	0f 84 b9 00 00 00    	je     7d4 <printf+0xd4>
  ap = (uint*)(void*)&fmt + 1;
 ...
 75c:	74 76                	je     7d4 <printf+0xd4>
    c = fmt[i] & 0xff;
 75e:	0f be cb             	movsbl %bl,%ecx
 761:	0f b6 c3             	movzbl %bl,%eax
    if(c=='\r') continue;
 764:	80 fb 0d             	cmp    $0xd,%bl
 767:	74 eb                	je     754 <printf+0x54>
    if(state == 0){
 769:	85 d2                	test   %edx,%edx
 76b:	74 c3                	je     730 <printf+0x30>
      }
    } else if(state == '%'){
 76d:	83 fa 25             	cmp    $0x25,%edx
 770:	75 e2                	jne    754 <printf+0x54>
      if(c == 'd'){
 772:	83 f8 64             	cmp    $0x64,%eax
 775:	0f 84 95 00 00 00    	je     810 <printf+0x110>
        printint(fd, *ap, 10, 1);
        ap++;
      } else if(c == 'x' || c == 'p'){
 77b:	81 e1 f7 00 00 00    	and    $0xf7,%ecx
 781:	83 f9 70             	cmp    $0x70,%ecx
 784:	74 5a                	je     7e0 <printf+0xe0>
        printint(fd, *ap, 16, 0);
        ap++;
      } else if(c == 's'){
 786:	83 f8 73             	cmp    $0x73,%eax
 789:	0f 84 91 00 00 00    	je     820 <printf+0x120>
          s = "(null)";
        while(*s != 0){
          putc(fd, *s);
          s++;
 ...
 792:	0f 84 e8 00 00 00    	je     880 <printf+0x180>
        putc(fd, *ap);
        ap++;
      } else if(c == '%'){
 798:	83 f8 25             	cmp    $0x25,%eax
 79b:	0f 84 cf 00 00 00    	je     870 <printf+0x170>
  write(fd, &c, 1);
 ...
 7c2:	e8 fc fd ff ff       	call   5c3 <write>
  for(i = 0; fmt[i]; i++){
 ...
 7cb:	83 c4 10             	add    $0x10,%esp
      ...
      state = 0;
 7ce:	31 d2                	xor    %edx,%edx
  for(i = 0; fmt[i]; i++){
 7d0:	84 db                	test   %bl,%bl
 7d2:	75 8a                	jne    75e <printf+0x5e>
    }
  }
}
 ...
 836:	0f 84 18 ff ff ff    	je     754 <printf+0x54>
...
free(void *ap)
{
 ...
 8c5:	a1 e4 0d 00 00       	mov    0xde4,%eax
{
 ...
 8e6:	73 10                	jae    8f8 <free+0x38>
{
 ...
 911:	5b                   	pop    %ebx
  freep = p;
 912:	a3 e4 0d 00 00       	mov    %eax,0xde4
}
 917:	5e                   	pop    %esi
 918:	5f                   	pop    %edi
 919:	5d                   	pop    %ebp
 ...
 937:	03 53 fc             	add    -0x4(%ebx),%edx
  freep = p;
 93a:	a3 e4 0d 00 00       	mov    %eax,0xde4
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 95d:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 960:	8b 3d e4 0d 00 00    	mov    0xde4,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 9a9:	8b 3d e4 0d 00 00    	mov    0xde4,%edi
 ...
 9bb:	e8 6b fc ff ff       	call   62b <sbrk>
  if(p == (char*)-1)
 ...
 9d2:	e8 e9 fe ff ff       	call   8c0 <free>
  return freep;
 9d7:	a1 e4 0d 00 00       	mov    0xde4,%eax
 ...
 a04:	89 72 04             	mov    %esi,0x4(%edx)
      freep = prevp;
 a07:	a3 e4 0d 00 00       	mov    %eax,0xde4
}
 ...
 a1e:	66 90                	xchg   %ax,%ax
    base.s.ptr = freep = prevp = &base;
 a20:	c7 05 e4 0d 00 00 e8 	movl   $0xde8,0xde4
 a27:	0d 00 00 
    base.s.size = 0;
 a2a:	bf e8 0d 00 00       	mov    $0xde8,%edi
    base.s.ptr = freep = prevp = &base;
 a2f:	c7 05 e8 0d 00 00 e8 	movl   $0xde8,0xde8
 a36:	0d 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 a39:	89 f8                	mov    %edi,%eax
    base.s.size = 0;
 a3b:	c7 05 ec 0d 00 00 00 	movl   $0x0,0xdec
 a42:	00 00 00 
    if(p->s.size >= nunits){
