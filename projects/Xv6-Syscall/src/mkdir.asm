...
free(void *ap)
{
 ...
 605:	a1 88 0a 00 00       	mov    0xa88,%eax
{
...
malloc(uint nbytes)
{
 ...
 69d:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 6a0:	8b 3d 88 0a 00 00    	mov    0xa88,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 6e7:	73 4f                	jae    738 <malloc+0xa8>
    ...
    if(p == freep)
 6e9:	8b 3d 88 0a 00 00    	mov    0xa88,%edi
 ...
 712:	e8 e9 fe ff ff       	call   600 <free>
  return freep;
 717:	a1 88 0a 00 00       	mov    0xa88,%eax
 ...
 744:	89 72 04             	mov    %esi,0x4(%edx)
      freep = prevp;
 747:	a3 88 0a 00 00       	mov    %eax,0xa88
}
 ...
 767:	0a 00 00 
    base.s.size = 0;
 76a:	bf 8c 0a 00 00       	mov    $0xa8c,%edi
    base.s.ptr = freep = prevp = &base;
 76f:	c7 05 8c 0a 00 00 8c 	movl   $0xa8c,0xa8c
 776:	0a 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 779:	89 f8                	mov    %edi,%eax
    base.s.size = 0;
 77b:	c7 05 90 0a 00 00 00 	movl   $0x0,0xa90
 782:	00 00 00 
    if(p->s.size >= nunits){
